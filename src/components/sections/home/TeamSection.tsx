...
function TeamCard({
  ...
  // The handle "decrypts" into place as the card connects.
  const handle = useScrambleText(m.handle, inView, {
    delay: 220 + i * 150,
    frames: 16,
    interval: 40,
  });
  ...
  return (
    <div
      className={`group relative cursor-default overflow-hidden rounded-lg border border-white/[0.07] bg-vos-surface opacity-0 transition-[border-color,box-shadow,transform] duration-[350ms] ease-[cubic-bezier(0.34,1.56,0.64,1)] hover:-translate-y-[3px] hover:border-vos-cyan/25 hover:shadow-[0_20px_60px_rgba(0,0,0,0.7),0_0_0_1px_rgb(var(--glow-cyan)/0.08),0_0_50px_rgb(var(--glow-cyan)/0.04)] ${
        inView ? "animate-[fade-up_0.65s_var(--ease-settle)_forwards]" : ""
      }`}
      style={{ animationDelay: `${0.15 + i * 0.15}s` }}
    >
      {/* connecting-to-node scan sweep, once on reveal */}
      {inView && (
        <span
          className="pointer-events-none absolute inset-0 z-[4] bg-[linear-gradient(to_bottom,transparent_42%,rgb(var(--glow-cyan)/0.5)_50%,transparent_58%)] animate-[scan-sweep_1s_var(--ease-settle)_both]"
      ...
      {/* avatar */}
      <div className="relative h-[260px] overflow-hidden bg-vos-ink max-[900px]:h-[200px] max-[480px]:h-[240px]">
        <Image
          src={avatar}
          alt={m.name}
          fill
        ...
        <div className="absolute right-4 top-4 z-[2] flex items-center gap-1.5 rounded-[3px] border border-vos-green/20 bg-black/[0.65] px-2.5 py-1 font-mono text-[9px] tracking-[0.12em] text-vos-green backdrop-blur-md">
          <span className="h-[5px] w-[5px] flex-shrink-0 rounded-full bg-vos-green shadow-[0_0_5px_rgb(var(--glow-green)/0.8)] animate-[pulse-soft_2s_ease-in-out_infinite]" />
          ACTIVE
        </div>
